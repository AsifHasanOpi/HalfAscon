...
def ascon_encrypt(key, nonce, associateddata, plaintext, variant="Ascon-128"):
    ...
    if variant in ["Ascon-128", "Ascon-128a"]:
        assert(len(key) == 8 and len(nonce) == 8)
...
def ascon_decrypt(key, nonce, associateddata, ciphertext, variant="Ascon-128"):
    ...
    assert variant in ["Ascon-128", "Ascon-128a", "Ascon-80pq"]
    if variant in ["Ascon-128", "Ascon-128a"]:
        assert(len(key) == 8 and len(nonce) == 8 and len(ciphertext) >= 8)
    if variant == "Ascon-80pq":
...
def ascon_initialize(S, k, rate, a, b, key, nonce):
    ...
    """
    iv_zero_key_nonce = to_bytes([k, rate * 8, a, b]) + key + nonce
...
def ascon_process_associated_data(S, b, rate, associateddata):
    ...
    if len(associateddata) > 0:
        ...
        for block in range(0, len(a_padded), rate):
            S[0] ^= bytes_to_int(a_padded[block:block+4])
            if rate == 16:
...
def ascon_process_plaintext(S, b, rate, plaintext):
    ...
    """
    p_lastlen = (2 * len(plaintext)) % rate
    ...
    for block in range(0, len(p_padded) - rate, rate):
        if rate == 8:
            S[0] ^= bytes_to_int(p_padded[block:block+4])
            ciphertext += int_to_bytes(S[0], 4)
        elif rate == 16:
    ...
    if rate == 8:
        S[0] ^= bytes_to_int(p_padded[block:block+4])
        ciphertext += int_to_bytes(S[0], 4)[:p_lastlen]
    elif rate == 16:
        S[0] ^= bytes_to_int(p_padded[block:block+8])
        S[1] ^= bytes_to_int(p_padded[block+8:block+16])
...
def ascon_process_ciphertext(S, b, rate, ciphertext):
    ...
    """
    c_lastlen = (2 * len(ciphertext)) % rate
    ...
    if rate == 8:
        c_padding1 = (0x8 << (rate-c_lastlen-1)*4)
        c_mask = (0xFFFFFFFF >> (c_lastlen*4))
        Ci = bytes_to_int(c_padded[block:block+4])
        plaintext += int_to_bytes(Ci ^ S[0], 4)[:c_lastlen]
        S[0] = Ci ^ (S[0] & c_mask) ^ c_padding1
    elif rate == 16:
        c_lastlen_word = c_lastlen % 8
        c_padding1 = (0x80 << (8-c_lastlen_word-1)*8)
        c_mask = (0xFFFFFFFF >> (c_lastlen_word*8))
        Ci = (bytes_to_int(c_padded[block:block+8]),
              bytes_to_int(c_padded[block+8:block+16]))
        plaintext += (int_to_bytes(S[0] ^ Ci[0], 4) +
                      int_to_bytes(S[1] ^ Ci[1], 4))[:c_lastlen]
        if c_lastlen < 8:
...
def ascon_finalize(S, rate, a, key):
    ...
    """
    assert(len(key) in [8, 20])
    S[rate//8+0] ^= bytes_to_int(key[0:4])
    S[rate//8+1] ^= bytes_to_int(key[4:8])
    S[rate//8+2] ^= bytes_to_int(key[8:] + zero_bytes(24-len(key)))

    ascon_permutation(S, a)

    S[3] ^= bytes_to_int(key[-8:-4])
    S[4] ^= bytes_to_int(key[-4:])
...
def ascon_permutation(S, rounds=1):
    ...
    for r in range(12-rounds, 12):
        ...
        S[4] ^= S[3]
        S[2] ^= S[1]
        T = [(S[i] ^ 0xFFFFFFFF) & S[(i+1) % 5] for i in range(5)]
        for i in range(5):
            S[i] ^= T[(i+1) % 5]
        S[1] ^= S[0]
        S[0] ^= S[4]
...
def bytes_to_state(bytes):
    return [bytes_to_int(bytes[4*w:4*(w+1)]) for w in range(5)]
...
def printstate(S, description=""):
    print(" " + description)
    print(" ".join(["{s:08x}".format(s=s) for s in S]))


def printwords(S, description=""):
    print(" " + description)
    print("\n".join(["  x{i}={s:08x}".format(**locals())
...
def demo_aead(variant):
    assert variant in ["Ascon-128", "Ascon-128a", "Ascon-80pq"]
    keysize = 20 if variant == "Ascon-80pq" else 8
    print("=== Encryption using HalfAscon ===".format(variant=variant))
    ...
    demo_print([("key", key),
                ...
                #("tag", ciphertext[-16:]),
                ("received_plaintext", receivedplaintext),
                ])
